:sectnums:
= Jakarta Transactions Specification, Version 1.2
...
* A Java mapping of the industry standard
X/Open XA protocol that allows a transactional resource manager to
participate in a global transaction controlled by an external
transaction manager
...
such an application server is an EJB server.
...
industry standard Enterprise JavaBeans (EJB) component architecture. In
...
between the Jata Transaction API and other Java APIs, including
Enterprise JavaBeans, JDBC 4.1 Standard Extension API, Java Message
Service, and Java Transaction Service.

=== Enterprise JavaBeans

The Enterprise JavaBeans architecture
requires that an EJB Container support application-level transaction
...
interface. Refer to the _JDBC 4.1 Specification_ for further details.
...
that uses a two-phase commit transaction protocol. In particular, a JMS
...
interface. Refer to the JMS 2.0 Specification for further details.
...
EJB container manages the transaction states for transactional EJB
...
commit/rollback in the onephase-optimized case).
...
transaction boundaries on CDI managed beans, as well as classes defined
as managed beans by the Java EE specification, at both the class and
method level where method level annotations override those at the class
level. See the EJB specification for restrictions on the use of
_@Transactional_ with EJBs. This support is provided via an
implementation of CDI interceptors that conduct the necessary
...
the expected behavior.
 ...
 _TransactionScoped_ annotated CDI managed
bean:
...
JDBC 4.1 driver may be used to access a relational database, a SAP
connector resource adapter may be used to access the SAP ERP system, and
...
specification of the JTA interefaces are required functionality and must
...
.  _Enterprise JavaBeansTM (EJB) 3.2
Specification_ , available at _http://jcp.org/en/jsr/detail?id=345_
.  _JDBCTM 4.1 Specification_ , available at
_http://jcp.org/en/ jsr/detail?id=221_
.  _JMS 2.0 Specification_ , available at
_http://jcp.org/en/jsr/detail?id=343_
.  _Contexts and Dependency Injection for the
Java EE Platform (CDI) 1.1 Specification_ , available at
_http://jcp.org/en/jsr/detail?id=346_
.  _Interceptor Specification_ , available at
_http://jcp.org/en/jsr/detail?id=318_
