...
boundaries and resolution programmatically.
...
Before enlisting resources, the transaction manager invokes the
`Transaction.isReadOnly` method to determine if the current transaction
is in read-only mode. If the transaction is in read-only mode, the
transaction manager must attempt to put the resource into read-only
mode by invoking the `ExtendedXAResource#setReadOnly` method on the
resource if it implements the `ExtendedXAResource` interface. If the
`ExtendedXAResource` cannot be put into read-only mode or the
`XAResource` does not implement the `ExtendedXAResource` interface,
...
* Prepare-phase ordering via `getPreparePriority()`
* Commit-phase ordering via `getCommitPriority()`

[[resource-ordering]]
==== Resource Ordering

Resource adapters may implement `jakarta.transaction.xa.ExtendedXAResource`
to indicate to the transaction manager the order in which the resource should
participate in the prepare and commit phases of the two-phase commit protocol.

===== Prepare-Phase Ordering

The `getPreparePriority()` method returns a `jakarta.transaction.xa.PreparePriority`
value. The transaction manager must call this method after invoking
`XAResource.end` for a given resource and before issuing `XAResource.prepare`
to any resource in the transaction, giving the resource manager the opportunity
to reflect on the work that was performed.

The transaction manager must respect the following rules:

* Resources returning `PreparePriority.EXCLUSIVE_LAST` must be prepared after
  all resources with `PreparePriority.EARLY` or `PreparePriority.NORMAL` priority.
  This supports the _Last Resource Commit Optimisation_ (LRCO), in which a
  non-XA resource participates by mapping its prepare call to a real commit.
* Resources returning `PreparePriority.EARLY` should be prepared before resources
  with `PreparePriority.NORMAL` priority. They may be prepared concurrently
  with each other to reduce overall latency.
* At most one resource per transaction may return `PreparePriority.EXCLUSIVE_LAST`.
  If a second such resource is enlisted, the transaction manager must throw
  `SystemException`.

===== Commit-Phase Ordering

The `getCommitPriority()` method returns a `jakarta.transaction.xa.CommitPriority`
value. The transaction manager must query this value before beginning the commit
phase.

The transaction manager must respect the following rules:

* Resources returning `CommitPriority.EXCLUSIVE_FIRST` must be committed before
  all resources with `CommitPriority.NORMAL` or `CommitPriority.LATE` priority.
* Resources returning `CommitPriority.LATE` must be committed after all resources
  with `CommitPriority.EXCLUSIVE_FIRST` or `CommitPriority.NORMAL` priority.
  Resources with `CommitPriority.LATE` may be committed concurrently with each other.
* At most one resource per transaction may return `CommitPriority.EXCLUSIVE_FIRST`.
  If a second such resource is enlisted, the transaction manager must throw
  `SystemException`.

A common use case for commit ordering is a transaction that involves both a
relational database (DB) and a message broker (JMS). Marking the JMS resource
`CommitPriority.LATE` ensures that messages become visible to downstream consumers
only after the database has committed and released its row locks, preventing the
race condition where a message is consumed before the corresponding data can be read.
...
The `isReadOnly` element of the annotation indicates if the
transaction is effectively read-only in that its only permitted
outcome is to roll back. Unlike rollback-only mode, the transaction must
continue to allow read operations. Such a transaction may fail early
...
allow commit if the `isReadOnly` element is `false` and disallow commit if
the `isReadOnly` element is `true`.
...
`begin(isReadOnly)` to start transactions in read-only mode
...
`isReadOnly` to allow starting transaction in read-only mode
when the value is `true`.
* Interface `jakarta.transaction.UserTransaction`, add method
...
for a transaction that does not allow commit.
