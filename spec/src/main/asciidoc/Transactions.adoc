...
The `allowCommit` element of the annotation indicates whether the
transaction is permitted to commit. When the transaction is not
permitted to commit, participating resources may consider the
transaction as effectively read-only. Such a transaction may fail early
...
be allow commit if the `allowCommit` element is `true` and disallow commit if
the `allowCommit` element is `false`.
...
`begin(allowCommit)` to start transactions in read-only mode
...
`allowCommit` to allow starting transaction in read-only mode
when the value is `false`.
* Interface `jakarta.transaction.UserTransaction`, add method
...
to cover the `allowCommit` element of the `Transactional` annotation.
