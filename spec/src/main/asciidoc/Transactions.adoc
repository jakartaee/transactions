...
interface is described in "`<<transaction-interface,See Transaction
Interface>>.`"
...
is not associated with any transaction, a `null` object reference is returned;
otherwise, a valid `Transaction` object is returned. The `Transaction` object
can later be passed to the `resume` method to reinstate the transaction context
...
(`XAResource.start(TMRESUME)`). Refer to "`<<resource-enlistment,See Resource Enlistment>>.`"
and "`<<transaction-association,See Transaction Association>>,`" for more details on resource
...
interface is described in "`<<xaresource-interface,See XAResource Interface>>.`"
...
The `isSameRM` method is discussed in "`<<identifying-resource-manager-instance,See Identifying Resource Manager
Instance>>.`"
...
* Added the following description to the end of "`<<resource-enlistment,See Resource
...
* "`<<xaresource-interface,See XAResource Interface>>`": The line "The transaction manager
