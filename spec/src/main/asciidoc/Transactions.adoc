...
NOTE: Support for `@Transactional` is only required in environments where Jakarta Context Dependency
Injection is available. In environments without CDI, the annotation is present in the API but
implementations are not required to provide runtime support for it. Applications that use
`@Transactional` without CDI present may encounter a `NoClassDefFoundError` or similar error at runtime.
...
NOTE: Support for `@TransactionScoped` is only required in environments where Jakarta Context Dependency
Injection is available. In environments without CDI, the annotation is present in the API but
implementations are not required to provide runtime support for it.
